...
{
    QSettings settings("Packt", "Current_Vision", this);
    ...
    if(!originalMat.empty())
    {
        if(!currentPlugin.isNull())
        {
            ...
            if(currentPluginInstance)
            {
                cv::TickMeter meter;
                meter.start();
                currentPluginInstance->processImage(originalMat, processMat);
                meter.stop();
        ...
        {
            processMat = originalMat.clone();
        }

        originalImage= QImage(originalMat.data, originalMat.cols, originalMat.rows,
                                                        originalMat.step, QImage::Format_RGB888);
        originalPixmap.setPixmap(QPixmap::fromImage(originalImage.rgbSwapped()));

        processedImage = QImage(processMat.data, processMat.cols, processMat.rows,
                                                              processMat.step, QImage::Format_RGB888);
...
{
    if(!ui->viewOriginalCheck->isChecked() && !processMat.empty())
    {
        QString fileName = QFileDialog::getSaveFileName(this, tr("Save Image"), QDir::currentPath(), "*.jpg;;*.png;;*.bmp");
        if(!fileName.isEmpty())
        {
            cv::imwrite(fileName.toStdString(), processMat);
        }
    }
