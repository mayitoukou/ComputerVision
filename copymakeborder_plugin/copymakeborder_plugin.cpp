...
void CopyMakeBorder_Plugin::setupUi(QWidget *parent)
{
    ...
    connect(ui->borderTypeComboBox, SIGNAL(currentIndexChanged(int)),
                                                                        this, SLOT(on_boderTypeComboBox_currentIndexChanged(int)));
}
...
void CopyMakeBorder_Plugin::processImage(const cv::Mat &inputImage, cv::Mat &outputImage)
{
    ...
    cv::copyMakeBorder(inputImage,
                    outputImage, top, bot, left, right,
                    ui->borderTypeComboBox->currentIndex());
}


void CopyMakeBorder_Plugin::on_boderTypeComboBox_currentIndexChanged(int index)
{
       Q_UNUSED((index))
       emit updateNeeded();
}
